...
vec2 res = vec2(screenWidth, screenHeight) / DOWN_SCALE;
...
float text(vec2 uv)
{
	...
	print_pos = vec2(screenWidth-STRWIDTH(7.0) - 2.0, screenHeight-STRHEIGHT(1.0) - 2.0);

	col += char(ch_F,uv);
	col += char(ch_P,uv);
	col += char(ch_S,uv);
	col += char(ch_col,uv);
	col += print_integer(FPS, 3, uv);
    ...
    TEXT_MODE = UNDERLINE;
    col += print_integer(screenWidth,0,uv); 
    col += char(ch_x,uv);
    col += print_integer(screenHeight,0,uv); 
    
    return col;
}

bool ASCIIScreenDraw() {
	float txtCoordX=mod(Texcoord.x*screenWidth ,FontX)*1/FontX;
	float txtCoordY=mod(Texcoord.y*screenHeight,FontY)*1/(FontY*FontSize);
...
void main()
{
	vec2 fragCoord = vec2(Texcoord.x*screenWidth, Texcoord.y*screenHeight);
	...
	if(pixel > 0.0) {
		...
		if( ASCIIScreenDraw() )
			outColor = imageLoad(frameBuffer, ivec2(fragCoord) );
		else
